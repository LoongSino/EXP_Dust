...
from resnet.model.benchmark import resnet50_benchmark
...
# 加载数据集
data_dir = '../../dust_images/RandomDataset_100'  # 替换为你的数据集路径
image_datasets = {x: datasets.ImageFolder(os.path.join(data_dir, x), data_transforms[x]) for x in ['train', 'val']}
dataloaders = {x: DataLoader(image_datasets[x], batch_size=32, shuffle=True, num_workers=8) for x in ['train', 'val']}
dataset_sizes = {x: len(image_datasets[x]) for x in ['train', 'val']}
class_names = image_datasets['train'].classes


# 加载预训练的ResNet50模型
model = resnet50_benchmark(num_classes=2)
...
def train_model(model, criterion, optimizer, num_epochs):
    ...
    # 保存损失图
    plt.savefig('../performance/benchmark_loss_001.png')
    plt.show()
    ...
    # 保存准确率图
    plt.savefig('../performance/benchmark_acc_001.png')
    plt.show()
...
if __name__ == '__main__':
    ...
    # 保存训练好的模型
    torch.save(model_trained.state_dict(), '../checkpoints/benchmark_dust_001.pth')
